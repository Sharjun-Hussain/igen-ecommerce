...
const DataTable = ({
    ...
    const allSelected = selectedRows.length > 0 && selectedRows.length === sortedData.length
    ...
    const ColumnFilter = ({ column }) => {
        ...
        return (
            <Select
                value={currentValue}
                onValueChange={(value) => setFilters(prev => ({ ...prev, [column.accessor]: value }))}
            >
                ...
                <Table>
                    ...
                    <TableBody>
                        {isLoading ? (
                            <TableRow>
                                <TableCell
                                    colSpan={columns.length + (bulkActions.length > 0 ? 1 : 0) + (onRowClick ? 1 : 0)}
                            ...
                            sortedData.map((item) => (
                                <TableRow
                                    key={item.id}
                                    className={onRowClick ? "cursor-pointer hover:bg-gray-50 dark:hover:bg-gray-800 border-0" : ""}
                                ...
                                >
                                    ...
                                    {columns.map(column => (
                                        <TableCell key={column.accessor} className={column.cellClassName || ""}>
                                            {column.cell ? column.cell(item) : getNestedValue(item, column.accessor)}
                                        </TableCell>
                                    ))}
                                                    ...
                                                    {bulkActions.length > 0 && (
                                                        <>
                                                            ...
                                                            {bulkActions.map(action => (
                                                                <DropdownMenuItem
                                                                    key={action.value}
                                                                    onClick={() => handleBulkActionClick(action.value, [item.id])}
                                                                    className={action.destructive ? "text-red-600" : ""}
                                                                >
                                                                    {action.label}
                                                                </DropdownMenuItem>
